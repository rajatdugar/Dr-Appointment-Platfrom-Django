...
def DOCSIGNUP(request):
    ...
    if request.method == "POST":
        ...
        if not all([first_name, last_name, username, email, mobno, specialization_id, password]):
            messages.warning(request,'Please fill all required registration fields')
        ...
        try:
            ...
            messages.warning(request,'Please select a valid specialization. Admin must add specializations first.')
            return redirect('docsignup')
        try:
            user = CustomUser(
            ...
            doctor.save()            
            messages.success(request,'Signup successfully. Please login with your email and password.')
